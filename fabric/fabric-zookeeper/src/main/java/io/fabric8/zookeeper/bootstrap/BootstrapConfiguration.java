...
public class BootstrapConfiguration extends AbstractComponent {
    ...
    public void createZooKeeeperServerConfig(CreateEnsembleOptions options) throws IOException {
        ...
        properties.put("dataBaseDir", options.getZooKeeperServerDataDir());
        properties.put("dataLogBaseDir", options.getZooKeeperServerDataLogDir());
        properties.put("ensembleId", "0000");
        properties.put("clientPort", Integer.toString(serverPort));
        properties.put("clientPortAddress", serverHost);
        properties.put("fabric.zookeeper.pid", "io.fabric8.zookeeper.server");
        Configuration config = configAdmin.get().getConfiguration(Constants.ZOOKEEPER_SERVER_PID, null);
        config.update(properties);
    }
