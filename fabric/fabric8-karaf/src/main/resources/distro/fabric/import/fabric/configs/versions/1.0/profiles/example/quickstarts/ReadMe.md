...
* [cbr.wiki](/fabric/profiles/example/quickstarts/cbr.wiki.profile) is the same Content Based Router using [Apache Camel](http://camel.apache.org/) where the Camel route is stored inside the wiki, so it can be easily changed via the Management Console
