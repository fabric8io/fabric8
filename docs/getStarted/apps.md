...
[packaged](http://repo1.maven.org/maven2/io/fabric8/devops/distro/distro/2.2.83/distro-2.2.83-templates.zip) to make them easy
...
* You can download the [fabric8 templates 2.2.83 distribution](http://repo1.maven.org/maven2/io/fabric8/devops/distro/distro/2.2.83/distro-2.2.83-templates.zip)
...
Download and unzip the [fabric8 templates 2.2.83 distribution](http://repo1.maven.org/maven2/io/fabric8/devops/distro/distro/2.2.83/distro-2.2.83-templates.zip).

e.g.

```
curl -o fabric8.zip http://repo1.maven.org/maven2/io/fabric8/devops/distro/distro/2.2.83/distro-2.2.83-templates.zip
unzip fabric8.zip
cd main
```
...
For example to install the [fabric8 console](console.html) then type:
		...
		oc process -v DOMAIN=$KUBERNETES_DOMAIN -f console-2.2.83.json | oc create -f -
...
Provides the base [fabric8 console](../console.html) at the `vagrant.f8` domain:
		...
		http://repo1.maven.org/maven2/io/fabric8/devops/apps/console/2.2.83/console-2.2.83-kubernetes.json \
		| oc create -f -
...
Provides centralised [Logging](logging.html) and [Metrics](metrics.html)
		...
		oc process -v DOMAIN=$KUBERNETES_DOMAIN -f \
		http://repo1.maven.org/maven2/io/fabric8/devops/packages/management/2.2.83/management-2.2.83-kubernetes.json \
		| oc create -f -
...
Provides just the centralised [Logging](../logging.html)
		...
		oc process -v DOMAIN=$KUBERNETES_DOMAIN -f \
		http://repo1.maven.org/maven2/io/fabric8/devops/packages/logging/2.2.83/logging-2.2.83-kubernetes.json \
		| oc create -f -
...
Provides just the centralised [Metrics](../metrics.html)
		...
		oc process -v DOMAIN=$KUBERNETES_DOMAIN -f \
		http://repo1.maven.org/maven2/io/fabric8/devops/packages/metrics/2.2.83/metrics-2.2.83-kubernetes.json \
		| oc create -f -
...
Provides the [fabric8 console](../console.html) and the [Integration Platform As A Service](../ipaas.html)
		...
		oc process -v DOMAIN=$KUBERNETES_DOMAIN -f \
		http://central.maven.org/maven2/io/fabric8/apps/ipaas/2.2.83/ipaas-2.2.83-kubernetes.json \
		| oc create -f -
...
The complete [Continuous Delivery](../cdelivery.html) installation.
		...
		oc process -v DOMAIN=$KUBERNETES_DOMAIN -f \
		http://repo1.maven.org/maven2/io/fabric8/devops/packages/cd-pipeline/2.2.83/cd-pipeline-2.2.83-kubernetes.json \
		| oc create -f -
...
If you have defined the [$KUBERNETES_DOMAIN](#setup-domain) environment variable then you can use the following command:
    ...
    mvn io.fabric8:fabric8-maven-plugin:2.2.83:create-routes

Otherwise you can be specific and specify the domain you wish to use:

    mvn io.fabric8:fabric8-maven-plugin:2.2.83:create-routes -Dfabric8.domain=my.acme.com
