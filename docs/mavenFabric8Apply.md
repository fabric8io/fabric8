...
You can use the maven property `fabric8.apply.domain` or the environment variable `$KUBERNETES_DOMAIN` to define the DNS host to expose services automatically using [OpenShift Routes](http://docs.openshift.org/latest/admin_guide/router.html)
...
To specify an explicit namespace and domain in recreate mode:
    ...
    -Dfabric8.apply.domain=foo.acme.com -Dfabric8.apply.namespace=cheese   
...
<td>If there is a route domain (see <code>fabric8.apply.domain</code>) then this option will create an <a href="http://docs.openshift.org/latest/admin_guide/router.html">OpenShift Route</a> for each service for the host expressio: <code>${serviceName}.${fabric8.apply.domain}</code>. Defaults to <code>true</code>.</td>
</tr>
<tr>
<td>fabric8.apply.domain</td>
