...
1. Change your working directory to `quickstarts/karaf/cxf/secure-soap` directory.
...
1. It is assumed that you have already created a fabric and are logged into a container called `root`.
1. Create a new child container and deploy the `quickstarts-karaf-cxf-secure.soap` profile in a single step, by entering the
...
1. Select `quickstarts` --> `karaf` --> `cxf` --> `secure-soap`
